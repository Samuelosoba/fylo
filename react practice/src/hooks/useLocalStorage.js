...
export default function useLocalStorage(key, defaultValue) {
  const [state, setState] = useState(() => {
    //retrive saved items from local storage via key, if we have an item saved, we deserialze it using json parse, otherwise we return the default value we initialized it with
